...
import numpy as np
import math
from KEF.Implementations.ExternalImplementations.GDM_Imagine.gammagwr import GammaGWR
...
class EpisodicGWR(GammaGWR):
    ...
    def init_network(self, ds, e_labels, num_context, max_nodes, age=600):
    ...
    def replay_samples(self, size):
        ...
        r_labels = np.zeros((self.num_nodes, len(self.num_labels), size))
        for i in range(0, self.num_nodes):
            for r in range(0, size):
                ...
                for l in range(0, len(self.num_labels)):
                    r_labels[i, l, r] = np.argmax(self.alabels[l][int(samples[r])])
    ...
    def train_egwr(self, ds_vectors, ds_labels, epochs, a_threshold, beta,
        ...
        self.tau_b = 0.3
        self.tau_n = 0.1
        # self.max_nodes = 2 * self.samples # OK for batch, bad for incremental
