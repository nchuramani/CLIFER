...
def face_embedding(vgg_weights, images):
    ...
    @param images: tensor of size [batch_size, imgsize, imgsize, 3]
    """
...
def face_features(vgg_weights, images):
    images = (images + 1) / 2 * 255
    net = vgg_face(vgg_weights, images)
    # print(net.keys())
    return tf.keras.layers.Flatten()(net['fc8'])


def vgg_face(data, input_maps, extract=False):
    """
